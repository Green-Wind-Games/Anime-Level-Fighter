...
function release_grab(_frame,_x,_y,_xspeed,_yspeed,_damage,_attacktype,_hiteffect,_hitanim) {
	if frame >= _frame {
		with(grabbed) {
			grabbed = noone;
			...
			x = other.x + (_x * other.facing);
			y = other.y + _y;
			get_hit(other,_damage,_xspeed,_yspeed,_attacktype,_hiteffect,_hitanim);
		}
		grabbed = noone;
	}
}