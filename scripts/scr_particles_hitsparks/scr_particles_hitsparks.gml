...
part_type_speed(hitspark_light,5,10,0,0);
...
part_type_life(hitspark_heavy,20,30);
part_type_size(hitspark_heavy,0.1,0.2,-1/20,0);
part_type_orientation(hitspark_heavy,0,0,0,0,true);
part_type_direction(hitspark_heavy,0,360,0,0);
part_type_speed(hitspark_heavy,5,10,0,0);
...
part_type_size(slashspark_light,0.2,0.3,-1/20,0);
...
part_type_life(slashspark_medium,10,15);
part_type_size(slashspark_medium,0.3,0.4,-1/20,0);
...
part_type_life(slashspark_heavy,10,15);
part_type_size(slashspark_heavy,0.4,0.5,-1/20,0);
		...
		if !_guard {
			switch(_hiteffect) {	
				...
				if _strength < attackstrength.medium {
					...
					create_particles(_x1,_y1,_x2,_y2,hitspark_light,20);
				}
				else if _strength < attackstrength.heavy {
					_sound = snd_punch_hit_medium;
					create_particles(_x1,_y1,_x2,_y2,hitspark_medium,30);
				}
				else if _strength < attackstrength.super {
					_sound = snd_punch_hit_heavy;
					create_particles(_x1,_y1,_x2,_y2,hitspark_heavy,40);
				}
				else if _strength < attackstrength.ultimate {
					_sound = snd_punch_hit_super;
					create_particles(_x1,_y1,_x2,_y2,hitspark_heavy,50);
				}
				else {
					_sound = snd_punch_hit_ultimate;
					create_particles(_x1,_y1,_x2,_y2,hitspark_heavy,60);
				}
				break;
				...
				if _strength < attackstrength.medium {
					...
					create_particles(_x1,_y1,_x2,_y2,slashspark_light);
				}
				else if _strength < attackstrength.heavy {
					_sound = snd_slash_hit_medium;
					create_particles(_x1,_y1,_x2,_y2,slashspark_medium);
				}
				else if _strength < attackstrength.super {
					_sound = snd_slash_hit_heavy;
					create_particles(_x1,_y1,_x2,_y2,slashspark_heavy);
				}
				else if _strength < attackstrength.ultimate {
