...
part_type_speed(hitspark_light,10,20,0,0);
...
part_type_size(slashspark_light,0.5,0.5,0,0);
...
part_type_size(slashspark_medium,1,1,0,0);
...
part_type_size(slashspark_heavy,2,2,0,0);
...
part_type_scale(piercespark_light,0.5,0.1);
part_type_size(piercespark_light,1,2,0,0);
...
part_type_scale(piercespark_medium,0.5,0.1);
part_type_size(piercespark_medium,2,3,0,0);
...
part_type_scale(piercespark_heavy,0.5,0.1);
part_type_size(piercespark_heavy,3,4,0,0);
...
part_type_size(guardspark,0.5,0.5,1/30,0);
part_type_scale(guardspark,0.5,1);
part_type_color1(guardspark,c_white);
part_type_alpha2(guardspark,1,0);
			...
			if !_guard {
				switch(_hiteffect) {	
					...
					if _strength < attackstrength.medium {
						...
						create_particle_premade(_x,_y,hitspark_heavy,60);
					}
					break;
					...
					if _strength < attackstrength.medium {
						...
						create_particle_premade(_x,_y,slashspark_light);
					}
					else if _strength < attackstrength.heavy {
						play_sound(snd_slash_hit_medium);
						create_particle_premade(_x,_y,slashspark_medium);
					}
					else if _strength < attackstrength.super {
						play_sound(snd_slash_hit_heavy);
						create_particle_premade(_x,_y,slashspark_heavy);
					}
					else if _strength < attackstrength.ultimate {
				...
				switch(_hiteffect) {
					...
					create_particle_premade(_x,_y,guardspark);
					break;
				}
			}
			break;
